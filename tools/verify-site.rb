...
%w[en fr].each do |language|
  ...
  check(home.css('script[data-discord-widget]').size == 1, 'Discord widget must initialize once')
  check(home.at_css('#sidebar script[data-discord-widget]'), 'Restore Discord state during sidebar parsing')
  check(home.css('script[src$="discord.min.js"]').empty?, 'Discord must not block page parsing on a network request')
  check(home.at_css('.discord-skeleton-avatar'), 'Missing first-visit avatar placeholder')
